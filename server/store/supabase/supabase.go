...
// Package supabase provides a Supabase storage driver for PayServer.
// Supabase is built on PostgreSQL, so this driver extends the postgres driver
// with Supabase-specific features like connection pooling configuration.
package supabase
...
// New creates a new Supabase storage driver
// URI format: supabase://user:pass@db.xxx.supabase.co:5432/postgres
// Or with pooler: supabase://user:pass@aws-0-us-west-1.pooler.supabase.com:6543/postgres
func New(c *store.Config) (store.Storage, error) {
	...
	dsn := c.URI
	dsn = strings.TrimPrefix(dsn, "supabase://")
	...
	// Supabase has specific pooling recommendations
	sqlDB, err := d.DB.DB()
	if err != nil {
		return nil, err
	}
	...
	// Supabase pro tier: max 200+ connections
